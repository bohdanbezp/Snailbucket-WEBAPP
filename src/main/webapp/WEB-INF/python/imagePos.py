...
class DrawChessPosition(object):
    ...
    def create_pieces(self):
        ...
        whites = 'KQBNRP'
        piece_images = dict(
            zip(whites, (Image.open(str(sys.argv[3]) + 'pieces/w%s.png' % p) for p in whites)))
        blacks = 'kqbnrp'
        piece_images.update(dict(
            zip(blacks, (Image.open(str(sys.argv[3]) + 'pieces/%s.png' % p) for p in blacks))))
