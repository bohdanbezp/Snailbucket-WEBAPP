...
class FicsProtocol(basic.LineReceiver):
    ...
    def post_login(self):
        """
        Code run after successful login, configures the connection.
        """
        try:
            ...
            for var, value in six.iteritems(vs):
                pending_defs.append(self.run_command("set %s %s" % (var, str(value))))

            # Add other setup commands as needed
            yield defer_util.gather_with_cancel(pending_defs)

            # Notify client after successful login
            yield self.client.on_login(self._fics_handle)

        except NotOnList as e:
            logger_lib.warning("%s: Channel not in list: %s" % (self.label, e))
        except Exception as e:
            logger_lib.error(
                "%s: Failure during login setup. Error:\n%s" % (
